      ...
      arr.forEach((item) => {
        ...
        if (item.type === 'text') {
          ...
          if (isRichTextContent) {
            ...
            // add code copy
            o.raw_tag = item.name
            if (item.name === 'code' && item.attribs && item.attribs.class) {
              if (item.attribs.class.indexOf('language') >= 0) {
                o.raw_tag = 'content_code'
                o.code_language = item.attribs.class.split('language-')[1]
              }
            }
            if (
              item.name === 'code' &&
